...
def MFUNetV5_trainer_mfdas1k(args, model, snapshot_path):
    ...
    for epoch_num in iterator:
        ...
        save_grad_cam_multi(
            ...
            all_val_spectrums,
            model,
            [model.attn4, model.attn4, model.decoder1],
            writer,
            save_CAM_interval,
            save_CAM_num
        )
...
if __name__ == '__main__':
    sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), '..')))
    from networks_architecture.mulit_feature_unet_v5_class_configs import get_MFUNetV5_config
    from networks_architecture.mulit_feature_unet_v5_class_modeling import MFUNetV5
    ...
    config_net = get_MFUNetV5_config()
    network = MFUNetV5(config=config_net)
    network = network.cuda()

    trainer = MFUNetV5_trainer_mfdas1k(args=config, model=network, snapshot_path=snapshot_path)