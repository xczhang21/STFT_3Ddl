...
def MFUNetV2_trainer_mfdas1k(args, model, snapshot_path):
    ...
    for epoch_num in iterator:
        ...
        save_grad_cam_multi(
            ...
            all_val_spectrums,
            model,
            [model.shared_encoder4, model.shared_encoder4, model.decoder1],
            writer,
            save_CAM_interval,
            save_CAM_num
        )
...
if __name__ == '__main__':
    sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), '..')))
    from networks_architecture.mulit_feature_unet_v2_class_configs import get_MFUNetV2_config
    from networks_architecture.mulit_feature_unet_v2_class_modeling import MFUNetV2
    ...
    config_net = get_MFUNetV2_config()
    network = MFUNetV2(config=config_net)
    network = network.cuda()

    trainer = MFUNetV2_trainer_mfdas1k(args=config, model=network, snapshot_path=snapshot_path)