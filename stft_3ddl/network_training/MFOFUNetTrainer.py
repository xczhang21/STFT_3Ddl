...
def MFOFUNet_trainer_mfdas1k(args, model, snapshot_path):
    ...
    for epoch_num in iterator:
        ...
        save_grad_cam_multi(epoch_num, all_val_spectrums, model, [model.encoder1_4, model.encoder2_4, model.decoder1], writer, save_CAM_interval, save_CAM_num)
...
if __name__ == '__main__':
    sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), '..')))
    from networks_architecture.mulit_feature_optional_fusion_unet_class_configs import *
    from networks_architecture.mulit_feature_optional_fusion_unet_class_modeling import MFOFUNet
    ...
    config_nets = [
        ...
        get_MFOFUNet_weight_add_config(alpha=0.1),
        get_MFOFUNet_learnable_add_config(init_alpha=0.5),
    ]
    for config_net in config_nets:
        network = MFOFUNet(config=config_net)
        network = network.cuda()
        trainer = MFOFUNet_trainer_mfdas1k(args=config, model=network, snapshot_path=snapshot_path)