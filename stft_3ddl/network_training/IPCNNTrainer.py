"""
IPCNNTrainer.py基本模仿自MFUNetTrainer.py
改动如下：
1. 方法名
2. 传入model的phase与intensity顺序,由[phase, intensity]改为[intensity, phase],因为IPCNN原文如此
3. grad_CAM所注意的网络层
"""
import logging
import sys
...
def IPCNN_trainer_mfdas1k(args, model, snapshot_path):
    ...
    for epoch_num in iterator:
        ...
        # 模仿MFUNetTrainer.py中的grad_cam使用方法
        """
        save_grad_cam_multi(
            epoch_num,
            all_val_spectrums,
            model,
            [
                model.i_branch.net[:2],
                model.p_branch.net[:2],
                model.classifier,
            ],
            writer,
            save_CAM_interval,
...
if __name__ == '__main__':
    sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), '..')))
    from networks_architecture.ipcnn_configs import get_IPCNN_config
    from networks_architecture.ipcnn_modeling import IPCNN
    ...
    config_net = get_IPCNN_config()
    network = IPCNN(config=config_net)
    network = network.cuda()

    trainer = IPCNN_trainer_mfdas1k(args=config, model=network, snapshot_path=snapshot_path)