...
def SwinTransformer_trainer_das1k(args, model, snapshot_path):
    ...
    for epoch_num in iterator:
        ...
        save_grad_cam(epoch_num, all_val_spectrums, model, model.layers[1].blocks[-1].norm2, writer, save_CAM_interval, save_CAM_num)
...
if __name__ == '__main__':
    sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), '..')))
    from networks_architecture.swintransformer_class_configs import get_SwinTransformer_config
    from networks_architecture.swintransformer_class_modeling import SwinTransformer
    ...
    config.net = get_SwinTransformer_config()
    config.net.in_channels = 1
    config.net.num_classes = 10
    config.net.img_size=256
    config.net.window_size = 8


    config.net_name = "SwinTransformer"
    config.seed = 1234
    ...
    network = SwinTransformer(config=config.net)
    network = network.cuda()

    trainer = SwinTransformer_trainer_das1k(args=config, model=network, snapshot_path=snapshot_path)
