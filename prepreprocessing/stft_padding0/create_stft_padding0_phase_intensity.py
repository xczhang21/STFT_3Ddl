...
if __name__ == '__main__':
    ...
    scales = [256, 128, 64]
    datas = []
    # 信号最长长度，这个值是在statistics_data_len.ipynb中统计出来的
    max_lengths = 49450
    ...
    for class_name in class_names:
        ...
        for iterfile_name in iterfile_names:
            ...
            for scale in scales:
                # 针对phase,后面会对intensity进行计算，虽然应该是没有必要，以防万一
                ...
                padded = False
                while(flag):
                    nperseg_i, noverlap_i, crop_i, flag, added_flag = temp_check_get_nperseg_noverlap(intensity, fs, window,
                                                                                                nperseg=nperseg,
                                                                                                noverlap=noverlap,
                                                                                                lfaxis=scale,
                                                                                                ltaxis=scale,
                ...
                datas.append({'id':iterfile_name, 'class':class_name, 'scale':scale,
                                    ...
                                    'signal_data':{'phase':phase, 'intensity':intensity}, 'crop':crop})
    ...
    for data in tqdm(datas, desc="数据处理进度"):
        for data_type in data_types:
            faxis, taxis, spectrum = get_spectrum_corp(data['signal_data'][data_type], fs=fs, window=window,
