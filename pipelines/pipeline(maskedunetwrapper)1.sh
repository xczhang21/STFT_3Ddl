...
# 配置参数列表
configs=(
    "maskedunetwrapper_ss64_train phase random0.5"
    "maskedunetwrapper_ss128_train phase random0.5"
    "maskedunetwrapper_ss256_train phase random0.5"
    "maskedunetwrapper_ss64_train intensity random0.5"
    "maskedunetwrapper_ss128_train intensity random0.5"
    "maskedunetwrapper_ss256_train intensity random0.5"
)
...
for i in {1..1}; do
    ...
    for config in "${configs[@]}"; do
        ...
        # 指定第0张显卡
        CUDA_VISIBLE_DEVICES=0 python ../stft_3ddl/train.py --train_config "${train_config}" --prepro_method "${prepro_method}" --to_config "${to_config}"
