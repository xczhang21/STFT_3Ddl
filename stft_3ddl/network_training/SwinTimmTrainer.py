...
def SwinTimm_trainer_das1k(args, model, snapshot_path):
    ...
    for epoch_num in iterator:
        ...
        save_grad_cam(epoch_num, all_val_spectrums, model, model.backbone.layers[0].blocks[-1].norm2, writer, save_CAM_interval, save_CAM_num)
...
if __name__ == '__main__':
    sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), '..')))
    from networks_architecture.swin_timm_class_configs import get_Swin_timm_config
    from networks_architecture.swin_timm_class_modeling import Swin_timm
    ...
    config.net = get_Swin_timm_config()
    config.net.in_channels = 1
    config.net.num_classes = 10
    config.net.img_size=256
    # config.net.model_name = 'swin_tiny_patch4_window7_224'
    config.net.model_name = 'swinv2_tiny_window8_256'
    config.net.pretrained = False
    config.net.enable_feat_hook = False

    config.net.replace_stride_with_dilation = None
    config.net.width_per_group = None
    config.net.zero_init_residual = None

    config.net_name = "SwinTimm"
    config.seed = 1234
    ...
    network = Swin_timm(config=config.net)
    network = network.cuda()

    trainer = SwinTimm_trainer_das1k(args=config, model=network, snapshot_path=snapshot_path)
