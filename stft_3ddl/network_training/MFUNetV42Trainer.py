...
def MFUNetV42_trainer_mfdas1k(args, model, snapshot_path):
...
if __name__ == '__main__':
    sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), '..')))
    from networks_architecture.mulit_feature_unet_v42_class_configs import get_MFUNetV42_config
    from networks_architecture.mulit_feature_unet_v42_class_modeling import MFUNetV42
    ...
    config_net = get_MFUNetV42_config()
    network = MFUNetV42(config=config_net)
    network = network.cuda()

    trainer = MFUNetV42_trainer_mfdas1k(args=config, model=network, snapshot_path=snapshot_path)