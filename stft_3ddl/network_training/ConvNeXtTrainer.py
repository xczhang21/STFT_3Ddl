...
def ConvNeXt_trainer_das1k(args, model, snapshot_path):
    ...
    for epoch_num in iterator:
        ...
        save_grad_cam(epoch_num, all_val_spectrums, model, model.features[1][-1].block[0], writer, save_CAM_interval, save_CAM_num)
...
ConvNeXt_trainer_das1k_aug = ConvNeXt_trainer_das1k
...
if __name__ == '__main__':
    sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), '..')))
    from networks_architecture.convnext_class_configs import get_ConvNeXt_config
    from networks_architecture.convnext_class_modeling import ConvNeXt
    ...
    config.dataset.root_path = "/home/zhang/zxc/STFT_3DDL/DATASETS/preprocessed_data/DAS1K/pi/matrixs/scale_256"
    ...
    config.net = get_ConvNeXt_config()

    config.net.in_channels = 2
    config.net.num_classes = 10

    config.net_name = "ConvNeXt"
    config.seed = 1234
    config.task_type = "cla_2in_ch"
    ...
    network = ConvNeXt(config=config.net)
    network = network.cuda()

    trainer = ConvNeXt_trainer_das1k(args=config, model=network, snapshot_path=snapshot_path)
