...
class ConvNeXt(nn.Module):
    def __init__(self, config, **kwargs: Any) -> None:
        super().__init__()
        block_setting = [
            CNBlockConfig(config.cnblock[0][0], config.cnblock[0][1], config.cnblock[0][2]),
            CNBlockConfig(config.cnblock[1][0], config.cnblock[1][1], config.cnblock[1][2]),
            CNBlockConfig(config.cnblock[2][0], config.cnblock[2][1], config.cnblock[2][2]),
            CNBlockConfig(config.cnblock[3][0], config.cnblock[3][1], config.cnblock[3][2]),
            ]
        ...
        layers.append(
            ...
            # )
            nn.Sequential(
                nn.Conv2d(in_channels, firstconv_output_channels, kernel_size=4, stride=4, padding=0, bias=False),
                nn.BatchNorm2d(firstconv_output_channels),
                nn.SiLU(inplace=True),
                # 可选：再来一点 Dropout2d 提高样本间多样性
                # nn.Dropout2d(p=0.05)
            )
        )
        ...
        self.classifier = nn.Sequential(
            ...
            # norm_layer(1024*10*10), nn.Flatten(1), nn.Linear(lastconv_output_channels, num_classes)
        )
        # # 怀疑是self.classifier的问题，将self.claasifier用下面的内容替换
    ...
    def _forward_impl(self, x: Tensor) -> Tensor:
        ...
        x = x / (x.std(dim=(2,3), keepdim=True) + 1e-5)
