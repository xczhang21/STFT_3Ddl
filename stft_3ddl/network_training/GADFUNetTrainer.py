...
def GADFUNet_trainer_das1k_gadf(args, model, snapshot_path):
    from datasets.dataset_das1k_gadf import das1k_gadf_dataset, RandomGenerator
...
if __name__ == '__main__':
    ...
    import argparse
    from datasets.datasets_config import get_das1k_gadf_phase_ssize64_config

    config = ml_collections.ConfigDict()

    config.base_lr = 0.01
    config.batch_size = 32
    config.dataset = get_das1k_gadf_phase_ssize64_config()
    ...
    trainer = GADFUNet_trainer_das1k_gadf(args=config, model=network, snapshot_path=snapshot_path)